...
(function( $, undefined ) {
	...
	var iemobile = navigator.userAgent.toLowerCase().search('windows phone os 7') > -1;
	if( ( $.mobile.fixedtoolbar.prototype.options.supportBlacklist() || iemobile ) && $.support.scrollTop ){
						...
						var resetPos = (function(){
							if( tbType === "header" ){
								...
								return function(){
									...
									toolbar.css( "bottom", page.outerHeight() - $( window).scrollTop() - $.mobile.getScreenHeight() - 34 + "px" );
									//toolbar.css("bottom", $(document).outerHeight() - $(window).scrollTop() - $.mobile.getScreenHeight() + "px");
									console.log(page.outerHeight() - $( window).scrollTop() - $.mobile.getScreenHeight() - 34);
